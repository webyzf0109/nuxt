...
// 异步加载高德地图js
export function initMapJs(that) {
    var url1 = 'https://webapi.amap.com/maps?v=1.4.15&key=1e4fbd6b99cfe7a05783b5852c364579&plugin=AMap.CitySearch'
    var url2 = 'https://webapi.amap.com/maps?v=1.4.15&key=1d08a17a2a1a2e865f97927357cb4f8f&plugin=AMap.CitySearch'
    var arr = [url1, url2];
    var script = document.createElement('script');
    script.charset = 'utf-8';
    script.src = arr[Math.floor((Math.random() * arr.length))];
...
//获取ip所在城市
export function initMapData(that) {
    ...
    citysearch.getLocalCity(async (status, result) => {
        if (status === 'complete' && result.info === 'OK') {
            if (result && result.city && result.bounds) {
                service.post('/ocs_cmcc_promotion/cmccAssist/loadAreaByProvinceCity', {
                    province: result.province,
                    city: result.city,
                }).then(res => {
                    if (res.data.level == 1) {
                        if (that.ruleForm.goodsProvinceCode) {
                            return;
                        }
                        that.ruleForm.goodsProvinceCode = res.data.code;
                        that.ruleForm.sendProvinceCode = res.data.code;
                        that.ruleForm.sendProvince = res.data.name;
                        that.userArea=res.data.name+' / ';
                        that.findExpressCity(res.data.code);
                    }
                    if (res.data.level == 2) {
                        that.ruleForm.goodsProvinceCode = res.data.code;
                        that.ruleForm.sendProvinceCode = res.data.code;
                        that.ruleForm.sendProvince = res.data.parentName;
                        that.userArea=res.data.parentName+' / '+res.data.name+' / ';
                        that.findExpressCity(res.data.code,res.data.parentCode,res.data.name);
                    }
                })
            }
        }
...
//协议
export function cmccflowerXY() {
    return `中国移动通信客户服务协议

甲方：（移动通信客户）

乙方：中国移动通信集团（号码归属地）有限公司
...
5、如遇政府主管部门对资费等相关内容的政策发生变化，乙方将根据相关要求，对原资费方案行相应调整。调整后方案的具体执行时间以乙方公示为准的内容。
...
15、乙方可能重新启用并销售部分其他客户曾使用过的移动电话号码。原使用该号码的客户（以下简称为“原机主”）如将该号码留作第三方业务的联系电话或者绑定第三方业务，且原机主未在注销前予以变更或解除绑定，该号码可能会收到第三方发给原机主的通知，或者因第三方的限制无法在解除绑定前完成新客户认证绑定。新客户需自行至第三方办理解除绑定等，乙方在合理限度内提供配合。

三、甲方的权利与义务
1、甲方办理入网手续时，需向乙方提供真实有效的身份证件原件。如甲方拒绝出示有效证件，拒绝提供其证件上所记载的身份信息，乙方有权不为其办理入网手续。如甲方入网登记资料发生变更，应及时通知乙方。
2、未满16周岁及其他无民事行为能力的用户,应由其法定代理人代为办理。
3、甲方应当按照约定的时间和方式及时、足额地向乙方交纳电信费用；甲方逾期不交纳电信费用的，乙方有权要求补交电信费用，并可以按照所欠费用每日加收3‰的违约金。
4、若甲方对乙方收取的移动通信费及代信息服务商收取的信息费存有异议，应在异议话费发生后五个月内向乙方提出(系统产生用户话单当月起后5个月，不含当月)，乙方应将有争议的数据保存到争议解决为止。
5、甲方要求终止提供移动通信服务时，应在结清所有费用后办理退网手续。
6、甲方承诺不利用移动通信网络发送违法信息或对其他客户的骚扰信息，甲方向第三方发送广告等信息，需经第三方同意认可。
...
4、甲方名下的移动电话号码逾期未交清费用的，乙方有权拒绝甲方以自己名义提出的其他移动通信业务申请（包括开办新的移动电话卡），缴费、话费查询除外。根据国家相关规定，甲方在中国移动办理有效使用的移动电话卡达到5张的，乙方不予为其开办新的移动电话卡。
...
8、因甲方（包括代理人）提供的客户资料不详、不实或变更后未通知乙方等原因，使乙方无法将服务（如话费单据等）提供给甲方，乙方不承担由此对甲方所造成的不良后果。甲方（包括代理人）未完全履行本协议约定义务，所提供客户资料不真实、不准确、不完整、超过有效期或无效的，乙方有权暂时中止履行本协议并催告甲方（包括代理人）补充提供相关资料。甲方在接到乙方催告后30天内拒不补充提供到位的，乙方有权终止本协议。
...
5、本协议项下发生的争议，双方可协商解决，协商不成的，双方可选择向电信管理部门申诉。
6、本协议自甲方确认阅读后自动生效。
`
}


export function cmccflowerYS(){
    return ` 版本生效日期：2020年1月15日
    
引言

移动与其运营主体（下称“我们”）非常重视用户（下称“您”）的隐私和个人信息保护。您在使用我们的产品与/或服务时，我们可能会收集和使用您的相关信息。我们希望通过《隐私权政策》（下称“隐私政策”）向您说明我们在您使用我们的产品与服务时如何收集、使用、保存、共享和转让这些信息，以及我们为您提供的访问、更新、删除和保护这些信息的方式。电话用户真实身份信息登记规定是为了规范电话用户真实身份信息登记活动，保障电话用户和电信业务经营者的合法权益，维护网络信息安全，促进电信业的健康发展而制定的法规，2013年6月28日，中华人民共和国工业和信息化部第2次部务会议审议通过，2013年7月16日中华人民共和国工业和信息化部令第25号公布，自2013年9月1日起施行。
本政策将帮助您了解以下内容：
1、我们如何收集和使用您的个人信息
...
我们在提供产品和服务的过程中可能涉及的个人信息包括：基本信息（包括个人姓名、住址、个人电话号码）；网络身份标识信息（包括系统账号、IP地址、邮箱地址及与前述有关的密码、口令、口令保护答案）；个人财产信息（交易和消费记录）；个人常用设备信息（包括硬件型号、设备MAC地址、操作系统类型、软件列表唯一设备识别码（如IMEI/android ID/IDFA/OPENUDID/GUID、SIM卡IMSI信息等在内的描述个人常用设备基本情况的信息）
个人敏感信息是指一旦泄露、非法提供或滥用可能危害人身和财产安全，极易导致个人名誉、身心健康受到损害或歧视性待遇等的个人信息，本隐私政策中涉及的个人敏感信息包括：您的个人身份信息（包括身份证）；其他信息（包括个人手机号码）。
我们基于以下目的收集和使用您的个人信息：
...
1、实现网上购物所必须的功能
（1）用户注册
您首先需要在线申请一张电信。当您注册时，您需要向我们提供您的姓名、手机号码、收货地址。这些信息均属于您的“账户信息”。在我们的功能支持的情况下，您可以补充信息，帮助我们为您提供个性化的商品推荐和更优的购物体验，但如果您不提供这些补充信息，不会影响您网上购物的基本功能。
（2）商品信息展示和搜索
为了让您快速地找到您所需要的商品，基于系统安全风控的考虑，也为了不断改进和优化我们的产品功能，我们可能会收集您使用我们的产品与/或服务的设备信息（包括设备名称、设备型号、设备识别码、操作系统和应用程序版本、语言设置、分辨率、服务提供商网络ID（PLMN））、浏览器类型来为您提供商品信息展示的最优方式。
...
（3）下达订单
当您下订单时，您需要在订单中填写、确认您的姓名、联系方式、收货地区、短信验证码、详细地址、身份证号，同时该订单中会载明订单号、购买的商品或服务信息、应支付的货款金额。
上述订单信息，将作为与您交易的凭证，用于对您的身份核验、确定交易、支付结算、配送。我们也基于此为您提供查询订单、提供客服咨询、售后服务。为了维护您、第三人及微卡平台的权益，我们还会使用您的订单信息来判断您的交易是否存在异常并采取合理措施。
（4）支付结算
在确认订单后，您可以通过电信合作的第三方支付机构所提供的支付服务。我们需要将您的订单号与交易金额信息与这些支付机构共享以实现您的支付并确认您的支付已完成。
（5）商品或服务的交付
当您成功支付订单金额后，我们将您的订单信息通知合作的销售商，由其安排第三方配送公司为您的订单进行配送和交付。您知晓并同意上述销售商及第三方配送公司会基于商品配送目的使用您的订单信息以保证您的订购的商品能够安全送达，我们已经通过协议要求这些第三方对您的订单信息予以严格保密。
第三方配送公司人员提供配送服务的同时，可能会基于法律法规的要求进行实名认证，此环节仅用户身份认证，且操作人员须遵守公司保密制度的规定。
（6）客服咨询与售后
我们在客服售后回访、应答您的咨询需求以及处理您的售后要求时会使用您的账号信息和订单信息。为保证您的交易安全，客服系统或客服人员会使用您的账号信息与您进行身份、订单信息核验，并在您提出变更个人信息时予以信息采集、更改及存储。
2、改进我们的产品与/或服务所必须的功能
我们可能会收集您的订单信息、浏览信息、您的兴趣爱好（可选）进行数据分析以形成用户画像，以匹配您可能感兴趣的商品以提高您的购买决策效率。我们还可能为了提供服务及改进服务质量的合理需要而获得您的其他信息，包括您与客服联系时您提供的相关信息，您参与问卷调查时的答复信息，以及您与我们的合作伙伴互动时我们获得的相关信息。对于我们收集到的信息，我们可能会将它们进行关联，以便我们能为您提供更好的服务
3、保障交易安全所必须的功能
为提高您使用我们的产品与/或服务时系统的安全性，预防不法分子利用钓鱼网站欺诈、黑客进攻、不法分子数据撞库等威胁您的会员卡交易风险及商城账户安全，我们可能会通过了解您的浏览信息、您常用的软件信息、设备信息等手段来判断您所面临的风险，并可能会记录一些我们认为有风险的链接（“URL”）；我们也会收集您的设备信息进行分析、统计流量并排查可能存在的风险、在您选择向我们发送异常信息时予以排查。
（二）您可选择是否授权我们收集和使用您的个人信息的情形
1、为使您购物更便捷或更有乐趣，我们在技术能力具备的情况下会开放并供您选择性使用以下附加功能，这些附加功能的使用与否取决于您是否做出明示性的选择。在使用这些功能的过程中，我们可能会收集和使用您的个人信息。如果您不提供这些个人信息，您依然可以进行网上购物，只是您将无法因为使用这些附加功能而获得更多的便捷和乐趣：
（1）基于位置信息的个性化推荐功能：我们会收集您的位置信息（我们仅收集您当时所处的地理位置来判断您所处的地点，并自动为您推荐您所在区域可以购买的商品或服务。比如向您推荐离您最近的可选的购物商场。
...
9、法律法规规定的其他情形。
（四）我们从第三方获得您个人信息的情形
我们可能从第三方获取您授权共享的账户信息（头像、昵称）如微信授权登录，使您可以通过第三方账户登录并使用我们的产品与服务。我们将会依据与第三方的约定、对个人信息来源的合法性进行确认后，在符合相关法律和法规规定的前提下，使用您的这些个人信息。
（五）您个人信息使用的规则
...
2、我们可能会将您的个人信息与我们的关联方（环球微卡）共享。但我们只会共享必要的个人信息，且受本隐私政策中所声明目的的约束。我们的关联方如要改变个人信息的处理目的，将再次征求您的授权同意。
...
如果您对我们的个人信息保护有任何疑问，可联系我们的客服予以反馈。如您发现自己的个人信息泄密，尤其是您的账户及密码发生泄露，请您立即与我们联系以便我们采取相应措施。
（二）您个人信息的保存
...
（2）您无法访问和更正的个人信息：除上述列明可查询及修改的信息外，您的部分个人信息暂无法为您提供访问和更正的服务，这些信息主要是为了提升您的用户体验和保证交易安全所收集的您的设备信息。上述信息我们会在您的授权范围内进行使用，您可联系我们进行删除或做匿名化处理。
2、删除您的个人信息
...
（4）如果我们终止服务及运营。
若我们决定响应您的删除请求，我们还将同时通知从我们获得您的个人信息的实体，要求其及时删除，除非法律法规另有规定，或这些实体另行获得您的独立授权。当您从我们的服务中删除信息后，我们可能不会立即备份系统中删除相应的信息，但会在备份更新时删除这些信息。
3、注销账户
您可以联系我们的客服要求注销您的账户，您注销账户后，我们将停止为您提供产品与/或服务，并依据您的要求，除法律法规另有规定外，我们将删除您的个人信息或匿名化处理，并不再进行任何技术利用。
注销本账户后，视为您放弃以下权益：
（1）账户信息、订单信息、会员权益、虚拟资产；
（2）会员卡号、验证手机或邮箱；
（3）解除与第三方授权或绑定关系；
您理解并同意，我们无法协助您重新恢复前述服务，您注销账户的行为会给您的售后维权带来诸多不便，请您在操作之前，确保您的账户无任何纠纷，并自行备份账户相关的所有信息和数据，下载并保存好订单商品和服务的交易凭证、票据（如电子发票）等资料。
4、拒绝推送
如果您不想接受我们给您发送的促销信息，您可以随时回复“TD”来取消我们给您发送的手机促销短信。
您在行使相关权利时遇到问题或有任何意见建议，您均可以通过我们的在线客服与我们联系。为了保障安全，我们可能需要您提供书面请求，或以其他方式证明您的身份，我们将在收到您反馈并验证您的身份后的30天内答复您的请求。对于您合理的请求，我们原则上不收取费用，但对多次重复、超出合理限度的请求，我们将视情收取一定成本费用。对于那些无端重复、需要过多技术手段（例如，需要开发新系统或从根本上改变现行惯例）、给他人合法权益带来风险或者非常不切实际（例如，涉及备份磁带上存放的信息）的请求，我们可能会予以拒绝。
...
八、如何联系我们
1、如您对本隐私政策或您个人信息的相关事宜有任何问题、意见或建议，请通过访问我们的客服进行咨询。
一般情况下，我们将在30天内回复。如果您对我们的个人信息处理行为损害了您的合法权益，您还可以向市场监管、网信、电信、公安等监管部门进行投诉或举报。`
}

/**个人信息收集证明**/
export function cmccflowerPersonInfo(){
    return `关于信息收集使用规则公告

尊敬的客户：
《全国人民代表大会常务委员会关于加强网络信息保护的决定》、《电信和互联网用户个人信息保护规定》（工业和信息化部令第24号）和《电话用户真实身份信息登记规定》（工业和信息化部令第25号）等国家法律法规的要求，客户在中国联合网络通信有限公司各类营业网点（含自有营业厅、手机营业厅、网上营业厅、授权合作代理商等）办理固定电话、移动电话（含无线上网卡）入网、过户以及需要出示客户证件的有关业务时，客户应配合出示有效证件原件并进行查验、登记，登记信息包括姓名、证件类型、号码及地址等。同时，为更好地提供服务，需要客户提供如联系人、联系电话、通信地址、电子邮箱等信息。客户本人持有效证件可通过自有营业厅查询和/或更正本人信息，或登录手机营业厅查询本人信息。
...
export function saveToutiao(that, id) {
    let linkNum = getQueryVariable('no');
    let clickid = getQueryVariable('clickid');
    if (clickid) {
        let link = window.location.href;
        // let link=`https://10086.refectman.com/cmccflower5?channelId=1868&clickid=star-NjkwMTkyMDI1MDYzNjI3NDQ0M3w2OTAxNjQ3Nzc4MzIyMDU3OTkyfDE2MDY5Nzg1NDU4Nzk%3D&room_id=6901920250636274443`
        let linkReplace = link.replace(/%3D/, "=");
        let preUrl = 'https://ad.oceanengine.com/track/activate/?link=';
        let appendUrl = '&event_type=3';
        let url = preUrl + encodeURIComponent(linkReplace) + appendUrl;

        service.post('/ocs_cmcc_promotion/cmccAssist/saveToutiao', {
            linkNum: linkNum,
            url: url,
            orderId: id,
            type: 2 // 1 联通  2 移动  3 电信
        })
    }
}
...
export function commentElement() {
    return `
        <div class="comment">
    <img class="img-title" src="https://jmdangdang-1301739225.cos.ap-shanghai.myqcloud.com/20200918150527670211.png"
         alt="">
    <div id="section-box" v-cloak>
        <div class="item" v-if="commentList&&commentList.length>0" :class="{anim:animate==true}" v-for="(item,index) in commentList" :key="index">

          <!--<div v-if="item.headImg" class="img" :style="{backgroundImage: 'url(' + item.headImg + ')', backgroundSize:'contain'}"></div>-->
          <img class="img" :src="item.headImg" alt="">
          <div class="comment-itemFont">
            <p>{{item.nickName}}</p>
            <p>{{item.content}}</p>
          </div>
        </div>
    ...
    <div class="a" style="height:0"  >
      <img :src="item.headImg" alt="" v-for="(item,index) in commentListCopy" :key="index">
    </div>
  </div>
    `
}

export function getAreaPca() {
    return [
        {
            "childList": [
                {
